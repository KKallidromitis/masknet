...
_TRANSFORMS_WITH_MASKS = [
    "MaskRandomResizedCrop",
...
DEFAULT_TRANSFORM_TYPES = {
    ...
    "TRANSFORMS_WITH_MASKS": _TRANSFORMS_WITH_MASKS,
...
class SSLTransformsWrapper(ClassyTransform):
    ...
    def __init__(
        ...
        self.transforms_with_masks = transform_types["TRANSFORMS_WITH_MASKS"]
    ...
    def _is_transform_with_masks(self):
        """
        _TRANSFORMS_WITH_MASKS = ["MaskRandomResizedCrop","MaskRandomHorizontalFlip"]
        """
        return self.name in self.transforms_with_masks
    ...
    def _transform(self, sample):
        ...
        if self._is_grouping_transform():
            ...
            for idx in indices:
                if self._is_transform_with_masks():
                    output = self.transform(sample["data"][idx],sample["mask"][idx])
                else:
                ...
                if self._is_transform_with_labels():
                    ...
                    if self._is_transform_with_masks():
                        sample["data"][idx],sample["mask"][idx] = output
                    else:
        ...
        if self._is_transform_with_copies():
            ...
            data, label, data_valid, data_idx, mask = [], [], [], [], []
