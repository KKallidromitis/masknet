...
class BaseSSLMultiInputOutputModel(ClassyModel):
    ...
    * What inputs do `heads' operate on?
    ...
    def single_input_forward(self, batch, feature_names, heads, mask):
        """
        ...
        if mask!= None:
            if len(feats) == 1:
                feats,mask = feats[0],mask[0]
            feats = create_maskedfeats(mask,feats)
    ...
    def forward(self, batch, mask=None):
        ...
        return self.single_input_forward(batch, self._output_feature_names, self.heads, mask)
    ...
    def _get_masknet(self):
        ...
        class Net(nn.Module):
            ...
            def forward(self, x):
                y = self.norm(F.relu(self.conv1(x)))
                return y
