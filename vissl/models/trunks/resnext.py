...
class ResNeXt(nn.Module):
    ...
    def __init__(self, model_config: AttrDict, model_name: str):
        ...
        if self.trunk_config['DROP_LAST']==0:
            num_layers = len(resnetx_layers)
        else:
            num_layers = - self.trunk_config['DROP_LAST']
        
        self._feature_blocks = nn.ModuleDict(resnetx_layers[:num_layers])
        self.feat_eval_mapping = dict(resnetx_mapping[:num_layers])
