...
from vissl.utils.input_mask import create_maskedfeats
import numpy as np
...
@register_train_step("detcon_train_step")
def detcon_train_step(task):
    ...
    #print('step',sample['data'][0].shape,len(sample['data']))
    ...
    with grad_context, ddp_context, torch_amp_context:
        ...
        with PerfTimer("forward", perf_stats), record_function("forward"):
            ...
            #torch.save(sample['mask'][0]) 
            
            '''
            import wandb
            from vissl.utils.input_mask import convert_binary_mask,sample_masks
            mask = task.model.masknet.forward(sample['mask'][0].float())
            lamda = 0.01+task.iteration*0.01 ; mask = mask*lamda + sample["mask"][0]
            
            masked_image = wandb.Image(sample['input'][0], masks={ "ground_truth": { 
                "mask_data": sample['mask'][0][0].cpu().detach().squeeze(0).numpy()}, "predictions": {
                "mask_data": mask[0].squeeze(0).cpu().detach().numpy()}})
            
            wandb.log({"img_with_masks" : masked_image})
            
            mask = convert_binary_mask(mask)
            mask,mask_ids = sample_masks(mask)
            '''
            '''
            mask = torch.reshape(task.model.masknet.forward(torch.reshape(sample['mask'][0],
                                                                             (10, 16, 7,7)).float()),(10, 16, 49))
